...
这样检查你有没有权限看 title 很简单，甚至不用遍历 AST ，只需要套一个模板。转换后：

```graphql

query noteSearch($ID: ID!, $title: String) {
  User(ID: $ID) {
    Groups {
      ACI(permission: "dr") {
        ACL {
