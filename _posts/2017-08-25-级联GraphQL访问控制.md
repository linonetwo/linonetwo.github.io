...
query MyQuestion($token: String!, $title: String) {
  Question(key: "title", value: $title) {
    title
    answers
  }
}
...
这样检查你有没有权限看 title 很简单，只需要套一个模板。转换后得到一个 Permission Query：
  ...
  User(ID: $ID) {
    Groups {
      ACI(permission: "dr") {
        ACL {
          DSR {
            titleProperty: Property(key: "title", value: $title) {
      ...
      ACI(permission: "dr") {
        ACL {
          DSR {
            answersProperty: Property(key: "answers") {
              Object(type: "Question") {
                ID
              }
            }
...
这样我们会得到一个多层数组，经过 flattten（lodash）过后我们看看 Object ID 是不是一致的，一致就说明我们对于这个 Object 连接的两个 Property 都有读权限，如果其中一个没有，就返回 ownerInfo 信息。然后我们对于有读权限的 Property 再发一次查询，取得它们的数据。
...
(u:User)-[...]->(p2) // 我们没法在一次查询里用 GraphQL 表达出这种 AND 关系
...
```
