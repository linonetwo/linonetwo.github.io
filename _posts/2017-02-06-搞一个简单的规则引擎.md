...
当某个定时任务开始时，我们用这个 key 去获取其对应的 scale，然后用 id 和 scale（时间段）从时序数据库中读取这段时间的平均值，用来跑规则引擎。因为 key 过期之后就废了，读不了里面的值，我只好用一个过期时间较长（例如 2T + 10000ms）的 key' 来存放 scale。
...
vvvv flatMap(getRules) vvvvv
---r----r--r----r------r-|-> rulesListStream: Observable<Array<Rule>>
vvvv map(getAggregated) vvvv
---r----r--r----r------r-|-> aggregatedRulesListStream: Observable<Array<Rule>>
vvvvv map(getDeviceId) vvvvv
vvvvvvvvvvvvvv map(diffWithRedis) vvvvvvvvvvvvvvvv
...
vvv do(closeRedisTask) vvvv    vvv do(openRedisTask) vvvv

```

对于聚合规则带来的定时任务，Redis 中 key 的过期会带来一个流，最后用 id 和 value 执行 ```runRules```，如果规则匹配到一条险情，则调用 API 网关的一个 API，告诉它发生的险情：

```javascript
// Period Task Stream

---e----e--e----e------e-|-> redisKeyExpireStream: Observable<string>
vvvv flatMap(getScale) vvvvv
---s----s--s----s------s-|-> idAndScaleStream: Array<{deviceId: string, scale: string}>
vvvv flatMap(getValue) vvvvv
---v----v--v----v------v-|-> aggregatedValueStream: Array<{deviceId: string, value: number}>
vvvv flatMap(runRules) vvvvv
---r----r--r----r------r-|-> redisKeyExpireStream: Observable<?Result>
// type Result = {deviceId: string, message: string};
vvvvv filter(notNull) vvvvvv
--------r-------r--------|-> redisKeyExpireStream: Observable<Result>
vvvvv do(callbackAPI) vvvvvv

```

对于约每五分钟到达一次的非聚合任务，这样来：

```javascript
// Sync Rule Stream

---r----r--r----r------r-|-> deviceRawDataStream: Observable<DeviceRawData>
vvvvvvv map(getValue) vvvvvv
---v----v--v----v------v-|-> aggregatedValueStream: Array<{deviceId: string, value: number}>
vvvv flatMap(runRules) vvvvv
---r----r--r----r------r-|-> redisKeyExpireStream: Observable<?Result>
vvvvv filter(notNull) vvvvvv
